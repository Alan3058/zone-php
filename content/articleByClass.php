...
function getArticleByClass() {
	// 查询文章
	$articles = ArticleController::getPublicArticleInfoByClassId( $_GET ["i"] );
	foreach ( $articles as &$item ) {
		if ($item ["CLASS_NAME"] == "") {
			$item ["CLASS_NAME"] = "未分类";
		}
		$item ["CONTENT"] = Util::substring ( preg_replace ( "/<[^>]+>/", "", $item ["CONTENT"] ) );
		$item ["VIEW_ARTICLE_URL"] = $_SERVER ["PHP_SELF"]."?p=article&i=" . $item ["ID"];
	...
	return $smarty->fetch ( "articleByClass.ftl" );
}

print getArticleByClass ();
?>
